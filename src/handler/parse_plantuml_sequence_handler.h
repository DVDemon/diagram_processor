...
#include "../plantuml/plantuml_sequence.h"
...
class ParsePlantumlSequenceHandler : public Poco::Net::HTTPRequestHandler {
public:
    void handleRequest(Poco::Net::HTTPServerRequest& request,
        ...
        try {
            ...
            std::string jsonResult = plantuml::PlantUmlSequenceParser::parse(plantumlText);
            ...
            if (g_httpErrors) g_httpErrors->inc();
            logger.error("parse_plantuml_sequence: %s", e.displayText().c_str());
            logDuration(start, request, 400, logger);
            ...
            if (g_httpErrors) g_httpErrors->inc();
            logger.error("parse_plantuml_sequence: %s", e.what());
...
private:
    ...
    static void logDuration(Poco::Timestamp start, Poco::Net::HTTPServerRequest& request,
        ...
        logger.information("%d POST /api/v1/parse_plantuml_sequence from %s, %.2f ms",
