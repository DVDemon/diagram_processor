...
#include "../drawio/drawio_parser.h"
...
class ParseDrawioHandler : public Poco::Net::HTTPRequestHandler {
public:
    void handleRequest(Poco::Net::HTTPServerRequest& request,
        ...
        try {
            ...
            if (contentType.find("application/json") != std::string::npos && !bodyStr.empty()) {
                ...
                if (obj->has("text")) {
                    xmlContent = obj->getValue<std::string>("text");
                } else if (obj->has("xml")) {
                    xmlContent = obj->getValue<std::string>("xml");
                } else if (obj->has("content")) {
                    xmlContent = obj->getValue<std::string>("content");
                } else if (obj->has("drawio")) {
                    xmlContent = obj->getValue<std::string>("drawio");
                } else {
                    response.setStatus(Poco::Net::HTTPResponse::HTTP_BAD_REQUEST);
                    std::ostream& ostr = response.send();
                    ostr << R"({"error":"Request must contain 'text', 'xml', 'content' or 'drawio' field","success":false})";
                    if (g_httpErrors) g_httpErrors->inc();
            ...
            if (xmlContent.empty()) {
                response.setStatus(Poco::Net::HTTPResponse::HTTP_BAD_REQUEST);
                std::ostream& ostr = response.send();
                ostr << R"({"error":"DrawIO XML cannot be empty","success":false})";
                if (g_httpErrors) g_httpErrors->inc();
                logDuration(start, request, 400, logger);
                return;
            }

            std::string jsonResult = drawio::DrawioParser::parse(xmlContent);

            if (jsonResult.find("\"success\":false") != std::string::npos) {
                response.setStatus(Poco::Net::HTTPResponse::HTTP_BAD_REQUEST);
            } else {
            ...
            if (g_httpErrors) g_httpErrors->inc();
            logger.error("parse_drawio: " + e.displayText());
            logDuration(start, request, 400, logger);
            ...
            if (g_httpErrors) g_httpErrors->inc();
            logger.error("parse_drawio: " + std::string(e.what()));
...
private:
    ...
    static void logDuration(Poco::Timestamp start, Poco::Net::HTTPServerRequest& request,
        ...
        logger.information("%d POST /api/v1/parse_drawio from %s, %.2f ms",
